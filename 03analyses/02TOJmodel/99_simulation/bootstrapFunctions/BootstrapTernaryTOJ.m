...
function [estP_btst, minNLL, lb_95CI, ub_95CI] = BootstrapTernaryTOJ(s_unique, r_org, nT,...
...
for i = 1:numBtst
    ...
    nT_V1st_slc = sum(r_slc == 1,2)';
    nT_A1st_slc = sum(r_slc == 3,2)';
    ...
    nLL = @(p) -nT_A1st_slc * log(P_Afirst(s_unique, p(1), p(2), p(3), p(4)))' ...
    -nT_V1st_slc*log(P_Vfirst(s_unique, p(1), p(2), p(3), p(4)))'...
    -nT_simul_slc * log(P_simultaneous(s_unique, p(1), p(2), p(3), p(4)))';
