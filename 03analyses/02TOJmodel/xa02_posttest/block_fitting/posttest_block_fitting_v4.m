...
post_s_unique_ms = ExpInfo.SOA; % unique SOA levels, in ms
post_numTrials = ExpInfo.nTrials;

% arbituarily bin post-trials into 5 blocks
nblock = 5;
tperblock = 65;
b_post_numTrials = post_numTrials/nblock;
...
for b = 1:5
    ...
    % organize response
    for i = 1:length(post_s_unique_ms)
        iSOA = post_s_unique_ms(i);
    ...
    % convert to p
    post_pResp = post_respCount/b_post_numTrials;
    
    % convert s_unique from s to ms for the following fitting and plotting
    post_s_unique = post_s_unique_ms*1000;
    ...
    nLogL = @(p) -pre_nT_A1st*log(P_Afirst(pre_s_unique, p(1), p(3), p(4), p(5)))' ...
        ...
        * log(P_simultaneous(pre_s_unique, p(1), p(3), p(4), p(5)))'...
        ...
        -(repmat(b_post_numTrials ,size(post_nT_A1st)) - post_nT_A1st - post_nT_V1st)...
    ...
    for i = 1:numBtst
        %initialize resampled responses
        post_r_slc = NaN(length(post_s_unique), b_post_numTrials);
        %for each stimulus location, we resample the responses
        for j = 1:length(post_s_unique)
            %randomly select trial indices (indices are allowed to occur more
            %than once, since we resample with replacement).
            idx        = randi([1 b_post_numTrials],[1 b_post_numTrials]);
            %store the resampled responses
    ...
    % convert to p
    post_btst_p = {post_nT_Vfirst_slc./b_post_numTrials; post_nT_simultaneous_slc./b_post_numTrials; post_nT_Afirst_slc./b_post_numTrials};
    for c = 1:3
...
%     set(f2,'Position',[10 10 900 600])
    set(gca, 'FontSize', 15)
    sgtitle(['sub' num2str(subjID) ' adaptor SOA = ' num2str(ExpInfo.adaptor*1000) ' ms'])
