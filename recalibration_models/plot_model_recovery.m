...
%% optional: choose 1 sim model and multiple fit models to compare model predictions
...
for fit_m = [1,4]
    ...
    sgtitle(sprintf('sim-Model %i, fit model%i, logME %.2f', sim_m, fit_m, log_model_evidence(sim_m, fit_m, i_sample)))
    hold on
    plot(fit_pred{sim_m, fit_m, i_sample}.adaptor_soa, mean(fit_pred{sim_m, fit_m, i_sample}.pss_shift,2),'--or')
    ...
    title('pretest')
    plot(fit_pred{sim_m, fit_m, i_sample}.test_soa, fit_pred{sim_m, fit_m, i_sample}.pre_pmf','--or')
    plot(fake_pred{sim_m, fit_m, i_sample}.test_soa, fake_pred{sim_m, fit_m, i_sample}.pre_pmf,'k')

    for ses = 1:9
        subplot(2,5,ses+1);
        hold on
        plot(fit_pred{sim_m, fit_m, i_sample}.test_soa, squeeze(fit_pred{sim_m, fit_m, i_sample}.post_pmf(ses,:,:))','--or')
...
%% optinal: choose 1 sample, show 6 sim models fitted by 6 fit models
...
for sim_m = 1:6
    ...
    for fit_m = 1:6
        ...
        subplot(6,6, 6*(sim_m-1)+fit_m)
        hold on
        title(sprintf('Fit by M%i, logME%.2f', fit_m, log_model_evidence(sim_m, fit_m, i_sample)))
        ylabel(sprintf('Sim by M%i', sim_m))
        plot(fake_pred{sim_m, fit_m, i_sample}.adaptor_soa, mean(fake_pred{sim_m, fit_m, i_sample}.pss_shift, 2),'-ok')
        plot(fit_pred{sim_m, fit_m, i_sample}.adaptor_soa, mean(fit_pred{sim_m, fit_m, i_sample}.pss_shift,2),'--r')
        ylim([-200 200])
        xlim([-700 700])
        xticks([-700 0 700])
        yline(0)
...
saveas(gca, fullfile(out_dir, 'check_all_models'),'png');
