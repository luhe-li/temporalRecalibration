...
%     ExpInfo.deviceIndices = [0, 1]; % use internal display, speaker;
    ...
    sessioname                           = {'A','V'};
    out1FileName                         = ['discrimination_v5_sub', num2str(ExpInfo.subjID) '_' sessioname{ExpInfo.session}]; %create file name
   ...
   % define bias
    ...
    ExpInfo.VolumeRange                  = logspace(log10(AudInfo.standard/1.5), log10( AudInfo.standard*1.5), ExpInfo.intLevel); 
    ...
    % shuffle order
    ExpInfo.trialOrder                   = sort(repmat(1:ExpInfo.intLevel, [1,ExpInfo.numTrials]));
