...
% Adaptor
xline(adaptor_soa(exp_adp),'Color',cmp2(3,:),'LineWidth',2);
...
% Data
for ss = [1,3,2]
scatter(D(idx_exp_exp).data(exp_adp).pre_ms_unique, D(idx_exp_exp).data(exp_adp).pre_pResp(ss,:), dotSZ, 'LineWidth', lw,'MarkerEdgeColor',cmp2(ss,:));
saveas(gcf, fullfile(out_dir, ['f' num2str(ss)]), 'pdf');
end
...
saveas(gcf, fullfile(out_dir, 'f4'), 'pdf');
...
saveas(gcf, fullfile(out_dir, 'f5'), 'pdf');
...
% Save figure
saveas(gcf, fullfile(out_dir, 'f6'), 'pdf');