function [out, out_sd] = nll_fixed_sym(freeParam, model, data)
...
if strcmp(model.mode, 'initialize')
    ...
    if strcmp(model.mode, 'optimize')
        ...
        for t    = 1:model.expo_num_sim
            ...
            tau_shift(:, t)  = sim_recal_fixed(model.expo_num_trial, adaptor_soas, ...
    ...
    elseif strcmp(model.mode,'predict')
        ...
        for t    = 1:model.expo_num_sim
            ...
            out.tau_shift(:,t) = sim_recal_fixed(model.expo_num_trial, out.adaptor_soa, ...
                tau, sigma_a, sigma_v, criterion, alpha);
