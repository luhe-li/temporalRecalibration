...
% Exposure phase: participants will be exposed to a train of 20
% audiovisual stimulus pairs with a consistent temporal discrepancy between
% them (adaptor lag fixed within a session).
...
% the other half were auditory stimuli with higher volume with independent
% probability.

% This version uses a concurrent task, which requires participants to
% respond ASAP if they detect an oddball. They don't have to make response
% when there is no oddball. If the response is too late to be recorded, it
...
%     ExpInfo.deviceIndices = [0, 1, 0]; % use internal display, speaker, keyboard;
    ...
    out1FileName                            = ['practice_exposure_sub', num2str(ExpInfo.subjID) '_session' num2str(ExpInfo.session)]; %create file name
...
%     [windowPtr,rect] = Screen('OpenWindow', ExpInfo.deviceIndices(1), [255, 255, 255] * pblack, [100 100 1000 600]); % for testing
    ...
    for i                                   = 1:ExpInfo.nTTrials
        ...
        if rem(i,ExpInfo.numTrialsinBlock)      ==0 && i~=ExpInfo.nTTrials
            ...
            [hitRate, FA]    = summarizeBlockPerformance(i - ExpInfo.numTrialsinBlock + 1, i, ExpInfo.idxOddballA, ExpInfo.idxOddballV, Response.oddball);
