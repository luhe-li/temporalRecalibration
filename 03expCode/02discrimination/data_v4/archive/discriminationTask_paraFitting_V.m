...
% This script obtains visual JND at 75% accuracy. 
...
%% clean raw data
load('discrimination_sub99_V.mat')

levels = ExpInfo.VolumeRange;
s_unique = levels; % unique SOA levels
...
% nLL cost function
nLL = @(p) -nT_compMore * log(PF_weibull(s_unique, p(1), p(2), p(3), p(4)))'...
    -nT_standardMore * log(1 - PF_weibull(s_unique, p(1), p(2), p(3), p(4)))';
...
[estP_btst, minNLL, lb_95CI, ub_95CI, nT_compMore_slc] = BootstrapDiscrimination...
    (s_unique, r_org, numTrials, numBtst, PF_weibull, b_lb, b_ub, b_options);
...
% errorbar(s_unique, pResp(2,:), eb, 'Color', cMAP(1,:),'MarkerSize',4, 'LineWidth',1); 
%  look better
ylim([0.4 1.0])
