function x75 = getAudioJND_v2(subjID)
...
load(['discrimination_sub' num2str(subjID) '_A.mat'])
...
% nLL cost function
nLL = @(p) -nT_compMore * log(PF_log_weibull(s_unique, p(1), p(2), p(3), p(4)))'...
    -nT_standardMore * log(1 - PF_log_weibull(s_unique, p(1), p(2), p(3), p(4)))';
...
[estP_btst, minNLL, lb_95CI, ub_95CI, nT_compMore_slc] = BootstrapDiscrimination...
    (s_unique, r_org, numTrials, numBtst, PF_log_weibull, b_lb, b_ub, b_options);

% compute sem of bootstrapped comparison-more response at each intensity
% level
for j = 1:length(s_unique)
    r_level = nT_compMore_slc(:,j);
    SEM(j) = std(r_level)/sqrt(length(r_level));
%     [CI_lb(j), CI_ub(j)] = get95CI(nT_compMore_slc(:,j));
end
...
errorbar(s_unique, pResp(2,:), SEM, '.','Color', cMAP(6,:),'MarkerSize',4, 'LineWidth',2);
%  look better
