function x95 = getVisualJND(subjID)
...
%% clean raw data
flnm = [num2str(subjID) '_V'];
load(['discrimination_sub' num2str(subjID) '_V.mat'])

% % fake response
% Response.more = Shuffle([ones(1,70), 2*ones(1,70)]);
% ExpInfo.VolumeRange = logspace(log10(0.5), log10(1), 7);

levels = ExpInfo.ContrastRange;
...
for i = 1:length(levels)
    iLevel = levels(i);
    iResp = RespComp(VSinfo.shuffledIntensity == iLevel);
...
f1 = figure; hold on
l1 = scatter(s_unique, pResp(2,:),40,'MarkerFaceColor', cMAP(1,:),...
...
% PF_weibull = @(x, gamma, lambda, alpha, beta) gamma + (1 - lambda - gamma).* wblpdf(x, alpha, beta);

% check PMF with arbiturary parameters
% int_finer = linspace(s_unique(1), s_unique(end), 1000);
% P =[0.5, 0.01, 7, 2];
% P_comp_more = PF_cumGaussian(int_finer, P(1), P(2), P(3), P(4));
% figure; hold on
...
% nLL cost function
nLL = @(p) -nT_compMore * log(PF_cumGaussian(s_unique, p(1), p(2), p(3), p(4)))'...
    -nT_standardMore * log(1 - PF_cumGaussian(s_unique, p(1), p(2), p(3), p(4)))';
...
bestP = estP(idx,:);
fprintf('gamma = %4.2f lambda = %4.2f mu = %4.2f sig = %4.2f\n', bestP)

%% bootstrap to obtain error bars

% define bounds for each parameter: gamma, lambda, alpha, beta
b_lb = [ 0, 0, 0, 0];
b_ub = [1, 0.06, 10, 10];
...
P = bestP;
P_comp_more = PF_cumGaussian(int_finer, P(1), P(2), P(3), P(4));
l2 = plot(int_finer, P_comp_more,  'Color', cMAP(1,:), 'LineWidth',3);
errorbar(s_unique, pResp(2,:), pResp(2,:)-lb, ub-pResp(2,:), '.','Color', cMAP(1,:),'MarkerSize',4, 'LineWidth',2);
%  look better

ylim([0.4 1.0])
ylabel('p(comparison brighter)')
xlabel('comparison luminance')
title(['sub' flnm])
xticks(round(levels,2))
