...
% Fit M7 and M8: all sessions are fitted together, delta_AIC is summed across
% sessions for each model, each subject

% output
% AIC       : a vector
% min_NLL   : a vector
% estP      : a cell (size = 1 x num_model), each element is fitted
%             parameter (a vector) across sessions
...
n_init                                     = 10;
numP                                       = [12, 14]; %number of free parameters
num_sess                                   = 9; %number of sessions

%define upper and lower bounds
lb                                         = {[repmat(-250,[1,10]), 10, 10],... %M7
                                             [repmat(-250,[1,10]), 10, 10, 10, 10]}; %M8
ub                                         = {[repmat(250,[1,10]), 350, 450],... %M7
                                             [repmat(250,[1,10]), 350, 350, 450 450]}; %M8
...
M7_NLL = @(p) sum(arrayfun(@(idx) M7_nll(pre_nT_V1st{idx}, pre_nT_simul{idx}, pre_nT_A1st{idx}, ...
    post_nT_V1st{idx}, post_nT_simul{idx}, post_nT_A1st{idx}, ...
    [p(1), p(idx+1), p(11), p(12)]), 1:num_sess));

M8_nll                                        = @(r1, r2, r3, r4, r5, r6, p) ...
    -r1*log(P_Vfirst(pre_ms_unique, p(1), p(3), p(5), fix_lambda))'...
    -r2*log(P_simultaneous(pre_ms_unique, p(1), p(3), p(5), fix_lambda))'...
    -r3*log(P_Afirst(pre_ms_unique, p(1), p(3), p(5), fix_lambda))'...
    -r4*log(P_Vfirst(post_ms_unique, p(2), p(4), p(6), fix_lambda))'...
    -r5*log(P_simultaneous(post_ms_unique, p(2), p(4), p(6), fix_lambda))'...
    -r6*log(P_Afirst(post_ms_unique, p(2), p(4), p(6), fix_lambda))';

M8_NLL = @(p) sum(arrayfun(@(idx) M8_nll(pre_nT_V1st{idx}, pre_nT_simul{idx}, pre_nT_A1st{idx}, ...
    post_nT_V1st{idx}, post_nT_simul{idx}, post_nT_A1st{idx}, ...
    [p(1), p(idx+1), p(11), p(12), p(13), p(14)]), 1:num_sess));
...
for m                                      = 1:numM
    ...
    [min_NLL(1,m) idx]                              = min(ii_min_NLL);
    estP{1, m}                                       = ii_estP{idx};
    %compute the AIC/BIC
    AIC(1,m)                                        = 2*min_NLL(1,m) + 2*numP(m);
