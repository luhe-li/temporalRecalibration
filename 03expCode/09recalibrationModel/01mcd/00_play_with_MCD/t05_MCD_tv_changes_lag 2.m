...
for s = 1:numel(k)
    ...
    plot(soas, MCD_corr,'LineWidth',1.5, 'Color', clm(clm_i(s),:));

    subplot(1, 2, 2); hold on
    plot(soas, MCD_lag,'LineWidth',1.5,  'Color', clm(clm_i(s),:));

end

%% plot default parameter set
...
xlabel('SOA (s)')
title('corr')
plot(soas, MCD_corr,'c','LineWidth',1.5);
...
xlabel('SOA (s)')
title('lag')
plot(soas, MCD_lag,'c','LineWidth',1.5);