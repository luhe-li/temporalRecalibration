...
%% clean raw data
subjID = 99;
load(['pretest_sub' num2str(subjID) '.mat'])

s_unique = ExpInfo.SOA; % unique SOA levels
...
% compute sem of bootstrapped response at each SOA
btst_resp = {nT_Vfirst_slc;  nT_Afirst_slc; nT_simultaneous_slc};
for c = 1:3
    SEM(c,:) = getSEM(btst_resp{c}, s_unique);
end
...
for c = 1:3
    errorbar(s_unique, pResp(c,:), SEM(c,:), '.','Color',cMAP(c,:),'LineWidth',2, 'HandleVisibility','off');
end

fignm = ['pretest_sub', num2str(subjID)];
