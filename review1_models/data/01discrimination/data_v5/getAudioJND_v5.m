function [lower_volume, higher_volume, db_JND] = getAudioJND_v5(subjID)
...
load(['discrimination_v5_sub' num2str(subjID) '_A.mat']);
...
% s_unique: in corresponding decibel
levels = ExpInfo.VolumeRange; % logspace(log10(0.5/1.5), log10(0.5*1.5), 7; 
s_unique = round(linspace(58,65,7),3);% unique intensity levels
...
% db that's 1 JND lower should be standard - 1*JND, in db
standard_db = 61.5;
lower_db = standard_db - db_JND;
higher_db = standard_db + db_JND;

% convert db to volume
% we know the mapping function db = 20*log10(V1/V0), when V1 = 0.5, db =
% 61.5, therefore we can obtain V0
V1 = 0.5;
V0 = V1/(10^(standard_db/20));

% once we know V0 in the mapping function, we can compute V1 depending on
% the desired db: V1 = V0 * 10^(db/20)
lower_volume = V0 * 10^(lower_db/20);
higher_volume = V0 * 10^(higher_db/20);
