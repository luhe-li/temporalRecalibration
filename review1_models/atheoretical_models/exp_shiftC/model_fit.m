...
model.paraID     = {'\mu','\sigma_A','\sigma_V','c1_{pre}','dc_{post,1}','dc_{post,2}','dc_{post,3}','dc_{post,4}','dc_{post,5}','dc_{post,6}','dc_{post,7}','dc_{post,8}','dc_{post,9}','\lambda'};
...
for i_sub = 1:10
    ...
    model.mode       = 'optimize';
    funcNLL          = @(p) nll_exp_shiftC(p(1), p(2), p(3), p(4), p(5), p(6),p(7),p(8), p(9),...
