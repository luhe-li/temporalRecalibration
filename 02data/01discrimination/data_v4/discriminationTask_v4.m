...
%     ExpInfo.deviceIndices = [0, 1, 0]; % use internal display, speaker, keyboard;
    ...
    sessioname                           = {'A','V'};
    out1FileName                         = ['discrimination_sub', num2str(ExpInfo.subjID) '_' sessioname{ExpInfo.session}]; %create file name
   ...
   % define bias
    ...
    % intenisty range of comparison stimulus
    ExpInfo.VolumeRange                  = logspace(log10(AudInfo.standard), log10( AudInfo.standard*1.75), ExpInfo.intLevel); 
    ExpInfo.ContrastRange                = linspace(VSinfo.standard, VSinfo.standard*2, ExpInfo.intLevel);
    
    % numTotalTrials: total number of trials
    ExpInfo.numTotalTrials               = ExpInfo.intLevel * ExpInfo.numTrials;

    % shuffle order
    ExpInfo.trialOrder                   = Shuffle(repmat(1:ExpInfo.intLevel, [1,ExpInfo.numTrials]));
